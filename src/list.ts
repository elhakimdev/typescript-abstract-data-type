...
export interface AbstractList<List>{
  ...
  insert: (element: List extends any ? List : never, after?: List extends any ? List : never) => List | boolean | undefined;  
   ...
   */
  find(predicate: (value: List, index: number, obj: List[]) => unknown, thisArg?: any): List | undefined;
...
export class List<T> implements AbstractList<T extends any ? T : never> {
  ...
  public insert(element: (T extends any ? T : never) extends any ? T extends any ? T : never : never, after?: ((T extends any ? T : never) extends any ? T extends any ? T : never : never) | undefined): boolean | (T extends any ? T : never) | undefined {
    if(after){
  ...
  public appends(elements: (T extends any ? T : never)[]){
    for (const e of elements) { this.append(e) }
  }

  public find(predicate: (value: T extends any ? T : never, index: number, obj: (T extends any ? T : never)[]) => unknown){
      return this.context.find((e, i) => predicate(e, i, this.context))
  }

  private isEquals(obj: T extends any ? T : never, targetObj: T extends any ? T : never): boolean {
    // @ts-expect-error
